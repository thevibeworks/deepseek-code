// HTTP handlers for sessions. Thin: parse, delegate, serialize.
...
const { validateSession } = require("../lib/validators");
...
function makeHandlers(repo) {
  return {
    ...
    async put(req, res) {
      const session = fromWire(req.body ?? {});
      ...
      res.json(toWire(repo.put(session)));
    },
