// HTTP handlers for seriesSet. Thin: parse, delegate, serialize.
...
const { validateSeries } = require("../lib/validators");
...
function makeHandlers(repo) {
  return {
    ...
    async put(req, res) {
      const series = fromWire(req.body ?? {});
      ...
      res.json(toWire(repo.put(series)));
    },
